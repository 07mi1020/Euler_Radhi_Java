...
public class GridProduct {
	...
	public static long[][] Arr = {
			{ 8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12,
					50, 77, 91, 8 },
	...
	public static void main(String[] args) {
		System.out.println("Product of coefficients is " + greatestProduct());
	}

	private static long greatestProduct() {

		// Horizontal product where i is row and j is column
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 17; j++) {
				long product = 1;
				for (int count = 0; count < 4; count++) {
					product *= Arr[i][j + count];
				}
				if (product > max) {
					max = product;
				}
		...
		// Vertical Product where i is column and j is row
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 17; j++) {
				long product = 1;
				for (int count = 0; count < 4; count++) {
					product *= Arr[j + count][i];
				}
				if (product > max) {
					max = product;
				}
